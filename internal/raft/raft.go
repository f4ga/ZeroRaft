...
type RaftNode struct {
	...
	// Channels for timer management
	resetTimerCh chan struct{}
	voteRetryCh  chan struct{} // retry sending RequestVote when packet loss
	stopCh       chan struct{}
...
func NewRaftNode(id int, peers map[int]string, dataDir string, sendFunc func(string, interface{}) error) *RaftNode {
	...
	rn := &RaftNode{
		...
		voteRetryCh:  make(chan struct{}, 1),
...
func (rn *RaftNode) startElection() {
	...
	term := rn.currentTerm
	// Initialize leader state for when we win
	...
	for _, addr := range rn.peers {
		go func(addr string, args codec.RequestVote) {
			_ = rn.sendFunc(addr, args)
		}(addr, args)
	}
	// Schedule a retry after 100ms to cope with packet loss
	go func() {
		time.Sleep(100 * time.Millisecond)
		select {
		case rn.voteRetryCh <- struct{}{}:
		default:
		}
	}()
}
func (rn *RaftNode) handleRequestVote(args codec.RequestVote) codec.RequestVoteResponse {
	rn.mu.Lock()
	defer rn.mu.Unlock()
	if args.Term < rn.currentTerm {
		return codec.RequestVoteResponse{Type: "RequestVoteResponse", Term: rn.currentTerm, VoteGranted: false}
	}
	if args.Term > rn.currentTerm {
	...
	rn.persistStateLocked()
	rn.resetElectionTimer()
	return codec.RequestVoteResponse{Type: "RequestVoteResponse", Term: rn.currentTerm, VoteGranted: true}
}
func (rn *RaftNode) handleAppendEntries(args codec.AppendEntries) codec.AppendEntriesResponse {
	rn.mu.Lock()
	defer rn.mu.Unlock()
	if args.Term < rn.currentTerm {
		return codec.AppendEntriesResponse{Type: "AppendEntriesResponse", Term: rn.currentTerm, Success: false}
	}
	if args.Term > rn.currentTerm {
		rn.currentTerm = args.Term
		rn.state = Follower
	...
	return codec.AppendEntriesResponse{Type: "AppendEntriesResponse", Term: rn.currentTerm, Success: true}
}
func (rn *RaftNode) handleResponseVote(peerID int, resp codec.RequestVoteResponse) {
	rn.mu.Lock()
	defer rn.mu.Unlock()
...
func (rn *RaftNode) handleAppendEntriesResponse(peerID int, resp codec.AppendEntriesResponse) {
	rn.mu.Lock()
	defer rn.mu.Unlock()
	if rn.state != Leader {
		return
	...
	for {
		...
		if state == Leader {
			...
			case <-rn.resetTimerCh:
				// Leader ignores resetTimerCh (no election timer)
			case <-rn.stopCh:
				close(rn.stopDone)
				return
		...
		case <-rn.voteRetryCh:
			rn.mu.RLock()
			candState := rn.state
			votes := len(rn.votesReceived)
			majority := len(rn.peers)/2 + 1
			rn.mu.RUnlock()
			if candState == Candidate && votes < majority {
				// Re‑send RequestVote to all peers
				rn.mu.Lock()
				args := codec.RequestVote{
					Type:         "RequestVote",
					Term:         rn.currentTerm,
				...
				// Schedule another retry if still not enough votes
				go func() {
					time.Sleep(100 * time.Millisecond)
					select {
					case rn.voteRetryCh <- struct{}{}:
...
func convertToTransportEntries(entries []LogEntry) []codec.LogEntry {
	result := make([]codec.LogEntry, len(entries))
	for i, e := range entries {
		result[i] = codec.LogEntry{
			Index:   e.Index,
			Term:    e.Term,
			Command: e.Command,
		}
...
func (rn *RaftNode) HandleRequestVoteResponse(peerID int, resp codec.RequestVoteResponse) {
